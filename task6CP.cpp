...
string calculatehotelprices(string mon,int stays)
{
float priceapt=0.0,pricest=0.0,discpr=0.0;
string type;
...
if(mon=="july" ||mon=="July" ||mon=="august"||mon=="August"){
    ...
    priceapt=77*stays;
     if(stays>14){
    discpr=priceapt-(0.10*priceapt);}
}

if(mon=="may" ||mon=="May" ||mon=="october"||mon=="October"){
    pricest=50*stays;
    priceapt=65*stays;

    if(stays>7&&stays<=14){
       discpr=pricest-(0.05*pricest);
    }
    else if(stays>14){
       discpr=pricest-(0.30*pricest);
       discpr=priceapt-(0.10*priceapt);
    }
}
